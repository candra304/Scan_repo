...
messages = {
    "start_scan": {"id": "Masukkan URL repositori GitHub", "en": "Enter the GitHub repository URL"},
...
def try_decode_base64(line):
    """Attempt to decode potential base64 strings in a line."""
    suspicious_decoded = []
...
def detect_custom_sensitive_vars(repo_zip):
    """Detect custom sensitive variables in repository files."""
    custom_vars = set()
    var_patterns = [
        r'PRIVATE[_\-]?KEY[_\d]*',
        r'PVKEY',
...
def analyze_code(content, file_path, suspect_vars_dynamic):
    """Analyze code content for potential security issues."""
    alerts = []
    urls = []
    data_send_alerts = []
    variables_found = set()
...
def fetch_repo_zip(url, lang):
    """Download and extract GitHub repository as zip file."""
    try:
    ...
    for file_name in repo_zip.namelist():
        if file_name.endswith(('.py', '.js')):
            with repo_zip.open(file_name) as f:
                ...
                alerts, found_urls, data_alerts, vars_found = analyze_code(
                    content, file_name, suspect_vars_dynamic
                )
...
def print_url_statistics(urls, lang):
    """Print statistics about found URLs."""
    if not urls:
        log("no_urls", lang)
        return
...
def main():
    ...
    if repo_zip:
        ...
        if suspicious_urls:
            is_suspicious = True
        elif any('☢️' in line or '👩‍💻📤' in line for line in open(LOG_FILE, encoding='utf-8').readlines()):
            is_suspicious = True
